...
export default function SiteFooter() {
  return (
    <footer className="border-t border-white/5 bg-[#030712] text-white/60">
      <div className="mx-auto max-w-[1280px] px-6 pb-12 pt-20">
        <div className="mb-16 grid gap-6 rounded-3xl border border-white/5 bg-gradient-to-r from-white/[0.03] to-white/[0.01] p-8 lg:grid-cols-[1.3fr_1fr] lg:items-center">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.16em] text-indigo-400">Lidera Corporate Engine</p>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:justify-end">
            <Link
              href="/empresas"
              className="inline-flex items-center justify-center gap-2 rounded-xl bg-indigo-500 px-5 py-3 text-sm font-bold text-white shadow-lg shadow-indigo-500/20 transition-colors hover:bg-indigo-400"
            >
              Falar com comercial
            ...
            <Link
              href="/cursos"
              className="inline-flex items-center justify-center rounded-xl border border-white/10 px-5 py-3 text-sm font-bold text-white/70 transition-colors hover:border-white/20 hover:text-white"
            >
              Explorar Academy
            </Link>
          </div>
        ...
        <div className="grid gap-12 lg:grid-cols-[1.2fr_1fr_1fr_1fr]">
            ...
            <div className="flex flex-wrap gap-3">
              {socialLinks.map(({ href, icon: Icon, label }) => (
                <a
                  ...
                  className="inline-flex h-10 w-10 items-center justify-center rounded-xl border border-white/10 text-white/40 transition-all hover:border-indigo-500/30 hover:text-indigo-400"
                >
          ...
          {Object.entries(footerLinks).map(([title, links]) => (
            <div key={title}>
              <p className="mb-5 text-xs font-bold uppercase tracking-[0.16em] text-white/40">{title}</p>
        ...
        <div className="mt-14 flex flex-col gap-4 border-t border-white/5 pt-6 text-sm text-white/40 md:flex-row md:items-center md:justify-between">
