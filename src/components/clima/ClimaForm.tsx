...
function ScaleItem({
  ...
  const labels = CLIMA_ESCALA[scale]
  return (
...
export default function ClimaForm() {
  ...
  const total = CLIMA_TODOS_ITENS.length
    ...
    return (
      <div className="py-10 text-center">
        ...
        <p className="mx-auto mt-2 max-w-md text-sm text-[#64748B]">
          Obrigado por participar. Sua resposta é anônima e vai ajudar a melhorar o ambiente de
          trabalho. Pode fechar esta página.
        </p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-8">
      ...
      {/* eNPS */}
      <section>
        <h2 className="text-sm font-extrabold text-[#0F172A]">{CLIMA_ENPS_PERGUNTA}</h2>
        <div className="mt-3 flex flex-wrap gap-1.5">
          {Array.from({ length: 11 }, (_, n) => {
            const selected = enps === n
            return (
              <button
                key={n}
                type="button"
        ...
        {CLIMA_PERGUNTAS_ABERTAS.map((q) => (
          <div key={q.id}>
            ...
            <textarea
              ...
              placeholder="Opcional — pode escrever à vontade."
            />
          </div>
        ))}
      </section>
